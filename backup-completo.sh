...
do_themes(){ log "Categoria: themes"; local target="$CATEGORY_DIR/themes"; mkdir -p "$target"; local copied=0; if [[ -d "$HOME/.themes" ]]; then log "Copiando ~/.themes..."; copy_safe "$HOME/.themes" "$target"; copied=1; fi; if [[ -d "$HOME/.icons" ]]; then log "Copiando ~/.icons..."; copy_safe "$HOME/.icons" "$target"; copied=1; fi; if [[ -d "$HOME/Imagens/Wallpapers" ]]; then log "Copiando ~/Imagens/Wallpapers..."; copy_safe "$HOME/Imagens/Wallpapers" "$target" 2>/dev/null || true; copied=1; fi; [[ $copied -eq 1 ]] && log "themes: $(find "$target" -type f 2>/dev/null | wc -l) arquivos copiados" || warn "Nenhuma pasta de tema encontrada"; }
